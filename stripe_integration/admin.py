...
class StripeApiKeyAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'stripe_account', 'account', 'default_currency', 'is_active', 
        'is_primary', 'created_at'
    ]
    list_filter = ['is_active', 'is_primary', 'default_currency', 'created_at']
    search_fields = ['stripe_account', 'account__name', 'stripe_client_id']
    readonly_fields = ['created_at', 'updated_at']
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('account', 'stripe_account', 'stripe_client_id', 'default_currency')
        }),
        ('API Configuration', {
