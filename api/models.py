...
class Client(models.Model):
    ...
    onboarding_link = models.UUIDField(
        default=uuid.uuid4,
        unique=True,
        editable=False,
        help_text='Permanent UUID link for onboarding form access'
    )
    short_onboarding_link = models.TextField(
        null=True,
        blank=True,
        help_text='Shortened URL for onboarding form (generated by URL shortener)'
    )
    reviews_link = models.UUIDField(
        default=uuid.uuid4,
        unique=True,
        editable=False,
        help_text='Permanent UUID link for reviews form access'
    )
    short_reviews_link = models.TextField(
        null=True,
        blank=True,
        help_text='Shortened URL for reviews form (generated by URL shortener)'
    )
...
class CheckInSchedule(models.Model):
    """Scheduling configuration for check-in and reviews forms"""
    ...
    account = models.ForeignKey(Account, on_delete=models.CASCADE, db_column='account_id')
    schedule_type = models.CharField(max_length=20, choices=SCHEDULE_TYPE_CHOICES, null=True, blank=True)
    ...
    time = models.TimeField(help_text='Time to send emails (e.g., 09:00)')
    ...
    # Reviews-specific interval fields
    interval_type = models.CharField(
        max_length=10,
        choices=INTERVAL_TYPE_CHOICES,
        null=True,
        blank=True,
        help_text='Interval type for reviews: weekly or monthly'
    )
    interval_count = models.IntegerField(
        null=True,
        blank=True,
        help_text='Number of weeks/months between reviews (1-12)'
    )
    last_triggered_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='Last time the schedule was triggered (for weekly interval tracking)'
    )
